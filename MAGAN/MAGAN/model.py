...
class MAGAN(object):
    ...
    def __init__(self,
        ...
        self.xb1_pcs = xb1_pcs
        self.xb2_pcs = xb2_pcs
    ...
    def save(self, iteration=None, saver=None, sess=None, folder=None):
        ...
        savefile = os.path.join(folder, 'MAGAN_model_{}'.format(self.iteration))
    ...
    def _build_loss_G(self):
        ...
        losses.append(1 * tf.reduce_mean(self.correspondence_loss(tf.slice(self.xb1, [0,0], [-1,100]), self.Gb2, self.xb1_pcs)))
        losses.append(1 * tf.reduce_mean(self.correspondence_loss(self.xb2, tf.slice(self.Gb1, [0,0], [-1,100]), self.xb2_pcs)))
...
class Generator(object):
    ...
    def __call__(self, x, reuse=False):
        ...
        with tf.variable_scope(self.name):
            h1 = tf.layers.dense(x, 512, activation=self.activation, reuse=reuse, name='h1')
            h2 = tf.layers.dense(h1, 256, activation=self.activation, reuse=reuse, name='h2')
            h3 = tf.layers.dense(h2, 64, activation=self.activation, reuse=reuse, name='h3')
